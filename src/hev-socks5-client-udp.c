                                         ...
                                         HevSocks5Addr *addr)
{
    ...
    if (res < 0) {
        LOG_W ("%p socks5 client failed to convert UDP address", self);
        return -1;
    }

    fd = hev_socks5_socket (SOCK_DGRAM);
    if (fd < 0) {
        LOG_E ("%p socks5 client failed to create UDP socket", self);
        return -1;
    }
    ...
    if (res < 0) {
        LOG_W ("%p socks5 client failed to bind UDP socket", self);
